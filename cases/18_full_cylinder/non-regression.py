...
"""
Z3ST case: 18_full_cylinder

non-regression script
-----------------------
Analytical solution for radial temperature in a cylinder with uniform volumetric heat generation.
...
def analytical_thermal_stress(x, T):
    ...
    sigma_tt = - sigma_star * (1. - 3.*(x/Ro)**2)
    sigma_rr = - sigma_star * (1. - (x/Ro)**2)
...
r_T, T = average_section_radial(x_T, y_T, z_T, T_all, z_target=z_target, tol=z_tol, decimals=3)
...
sigma_rr_ana_th, sigma_tt_ana_th, sigma_zz_ana_th = analytical_thermal_stress(r_s, T)
